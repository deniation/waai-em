package com.asisst.waaiem.user.main.pages.home.adapter
...
class PostsRecyclerAdapter(
    ...
    private val allOptions: ConstraintLayout?,
    private val othersOptions: ConstraintLayout?,
    private val selfOptions: ConstraintLayout?,
    private val closeOthersOptions: ImageView?,
    private val closeSelfOptions: ImageView?,
    private val follow: ImageView?,
    private val unfollow: ImageView?,
    private val follow_count: TextView?,
    private val unfollow_count: TextView?,
    ...
    override fun onBindViewHolder(holder: PostsViewHolder, position: Int) {
        ...
        holder.postsViewBinding.postsMoreId.setOnClickListener {
            if (post_owner.equals(user!!)){
                if (allOptions?.visibility == View.VISIBLE){
                    val animation = AnimationUtils.loadAnimation(ctx, R.anim.fade_out)
                    allOptions?.animation = animation
                    allOptions?.visibility = View.GONE
                }else{
                    val animation = AnimationUtils.loadAnimation(ctx, R.anim.fade_in)
                   allOptions?.animation = animation
                   allOptions?.visibility = View.VISIBLE
                    selfOptions?.visibility = View.GONE
                    othersOptions?.visibility = View.VISIBLE
        ...
        closeSelfOptions?.setOnClickListener {
            val animation = AnimationUtils.loadAnimation(ctx, R.anim.fade_out)
            allOptions?.animation = animation
            allOptions?.visibility = View.GONE
        }

        closeOthersOptions?.setOnClickListener {
            val animation = AnimationUtils.loadAnimation(ctx, R.anim.fade_out)
            allOptions?.animation = animation
            allOptions?.visibility = View.GONE
        }
                ...
                if (databaseError?.message != null) {
                    ...
                    firebaseDatabase?.child("Notifications")?.child(post_owner!!)?.push()?.setValue(follow_map)?.addOnCompleteListener {
                        if (it.isSuccessful){
                            ...
                            follow_count?.visibility = View.GONE
                            unfollow?.visibility = View.VISIBLE
                            unfollow_count?.visibility = View.VISIBLE
        ...
        unfollow?.setOnClickListener {
            ...
            firebaseDatabase?.child("Followers")?.child(user!!)?.child(post_owner)?.removeValue()?.addOnCompleteListener {
                if (it.isSuccessful){
                    ...
                    follow_count?.visibility = View.VISIBLE
                    unfollow.visibility = View.GONE
                    unfollow_count?.visibility = View.GONE
